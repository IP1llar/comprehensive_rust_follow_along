...
// covering all the different possibility
// The `std::error::Error` trait makes it easy to create a trait object that can contain any error
use std::error::Error;
use std::fs;
use std::io::Read;
...
//
// Make sure to implement the `std::error::Error` trait when defining a custom error type so it can
// be boxed. But if you need to support the `no_std` attribute, keep in mind that the
// `std::error::Error` trait is currently compatible with `no_std` in nightly only.
