...
fn first_try_offset_differences<N>(offset: usize, values: Vec<N>) -> Vec<N>
where
    N: Copy + std::ops::Sub<Output = N>,
{
    ...
    let actual_offset = offset % values.len();
    let values_clone = values.clone();
    values.into_iter().enumerate().map(|(i, v)| values_clone[(i + actual_offset)%length] - v).collect()
}
