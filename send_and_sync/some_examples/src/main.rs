...
// - `MutexGuard<T: Sync>`: Uses OS level primitives which must be deallocated on the thread which
// created them
//
//
// # !Send + !Sync
...
// - `Rc<T>`: each `Rc<T>` has a reference to an `RcBox<T>`, which contains a non-atomic reference
// count
