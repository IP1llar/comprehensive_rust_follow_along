// If a data type stores borrowed data, it must be annotates with a lifetime
#[derive(Debug)]
...
// The annotation on `Highlight` enforces that the data underlying the contained `&str` lives at
// least as long as any instance of `Hightlight` that uses that data
//
...
// references themselves, in addition to the lifetime of the struct itself
// - Those are very advanced use cases
