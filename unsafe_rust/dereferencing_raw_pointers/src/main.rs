...
fn main() {
    ...
    // and properly aligned, the objects underlying the references from which they were obtained
    // are live throughout the whole unsafe block, and they are not accessed either through the
...
//
// In mst cases the pointer must also be properly aligned
//
// The "NOT SAFE" section gives an example of a common kind of YB bug: `*r1` has the `'static`
// lifetime, so `r3` has type `&'static String`, and thus outlives `s`. Creating a reference from a
// pointer requires great care
